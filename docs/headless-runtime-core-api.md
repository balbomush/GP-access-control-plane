...
1. Явный параметр или переменная окружения отключает Web UI.
2. Установщик не должен зависать на вопросе.
3. Core API bind по умолчанию остается локальным до решения auth-модели.

...
"""
    ...
    def test_metadata_publish_failure_restores_the_previous_complete_generation(self) -> None:
        import gp_control_plane.domain_sources._local as v2fly_local
        ...
        for writer_name in ("write_v2fly_group_manifest", "write_v2fly_catalog_cache"):
            with self.subTest(writer_name=writer_name), tempfile.TemporaryDirectory() as raw:
                ...
                manifest_before = v2fly_local.v2fly_group_manifest_path(state_dir).read_bytes()
                cache_before = v2fly_local.v2fly_catalog_cache_path(state_dir).read_bytes()

                with patch.object(v2fly_local, writer_name, side_effect=OSError(f"forced {writer_name} failure")):
                    with self.assertRaises(OSError):
                        prepare_v2fly_local_storage(
                            state_dir,
                            archive_fetcher=lambda: _v2fly_archive({"discord": "domain:discord.com\n"}),
                ...
                self.assertEqual(v2fly_local.v2fly_group_manifest_path(state_dir).read_bytes(), manifest_before)
                self.assertEqual(v2fly_local.v2fly_catalog_cache_path(state_dir).read_bytes(), cache_before)

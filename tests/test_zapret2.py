...
"""
    ...
    def test_strict_service_inactive_or_not_found_accepts_only_systemctl_inactive_codes(self) -> None:
        shell = _posix_shell()
        if shell is None:
            self.skipTest("requires a POSIX sh interpreter")

        helper = (Path(__file__).resolve().parents[1] / "scripts" / "gp-root-helper.sh").read_text(encoding="utf-8")
        function_body = helper.split("strict_service_is_inactive_or_not_found() {\n", 1)[1].split("\n}\n", 1)[0]
        rendered_function = "strict_service_is_inactive_or_not_found() {\n" + function_body.replace("\\$", "$") + "\n}\n"
        harness = f'''\\
systemctl() {{
  case "${{3:-}}" in
    inactive) return 3 ;;
...
{rendered_function}
strict_service_is_inactive_or_not_found inactive || exit 11
strict_service_is_inactive_or_not_found missing || exit 12
if strict_service_is_inactive_or_not_found unexpected; then exit 13; fi
if strict_service_is_inactive_or_not_found active; then exit 14; fi
exit 0
'''

        completed = subprocess.run([shell, "-c", harness], text=True, capture_output=True, check=False)

        self.assertEqual(completed.returncode, 0, completed.stderr)

    def test_generated_strict_runner_keeps_terminal_failure_evidence_before_publication(self) -> None:
        if os.name == "nt":
            self.skipTest("strict runner execution is verified on Linux boards; MSYS path translation is not a POSIX privilege model")
        shell = _posix_shell()
        if shell is None:
            self.skipTest("requires a POSIX sh interpreter")
        helper = Path(__file__).resolve().parents[1] / "scripts" / "gp-root-helper.sh"
        ...
        cases = (
            ("remote-mismatch", "b" * 40, "canonical candidate ref does not match expected SHA"),
            ("verification-fetch", expected_sha, "verification fetch failed"),
            ("strict-stage-fetch", expected_sha, "strict-stage fetch failed"),
        )
        for failure_case, remote_sha, expected_error in cases:
            with self.subTest(failure_case=failure_case), tempfile.TemporaryDirectory() as raw:
                root = Path(raw)
                fake_bin = root / "fake-bin"
                fake_bin.mkdir()
                git_log = root / "git.log"
                git_log_shell = shlex.quote(_posix_shell_path(git_log))
                (fake_bin / "git").write_text(
                    ...
                    f"  ls-remote) printf '%s\\t%s\\n' {remote_sha} refs/heads/dev ;;\n"
                    "  init) exit 0 ;;\n"
                    "  fetch)\n"
                    f"    case \"{failure_case}:$repo\" in\n"
                    "      verification-fetch:*/verify|strict-stage-fetch:*/repo) exit 1 ;;\n"
                    "    esac\n"
                    "    exit 0\n"
                    "    ;;\n"
                ...
                harness = '''\
. "$1"
STRICT_RUN_DIR="$2/runs"
STRICT_STAGE_DIR="$2/stages"
STRICT_BUNDLE_DIR="$2/bundles"
DISCOVERY_GATE_FILE="$2/discovery-update.lock"
load_strict_install_profile() {
  strict_install_user=gp
  strict_install_dir=/install
  strict_state_dir=/state
  strict_state_layout=external
  strict_state_relative=
  strict_data_root=
  strict_core_service=gp-control-plane-core.service
  strict_web_service=gp-control-plane-web.service
  strict_install_web=off
  strict_core_env_file=/etc/default/gp-control-plane-core
  strict_web_env_file=/etc/default/gp-control-plane-web
}
prepare_strict_state_layout() { :; }
ensure_strict_root_dir() { /bin/mkdir -p "$1"; }
date() { printf '20260101T000000Z\\n'; }
chown() { :; }
chmod() { :; }
systemd-run() { :; }
queue_strict_update refs/heads/dev "$3"
'''
                generated = subprocess.run(
                    [shell, "-c", harness, "strict-runner-generation", _posix_shell_path(library), _posix_shell_path(root), expected_sha],
                    env={
                        **os.environ,
                ...
                unit_match = re.search(r"^unit=(.+)$", generated.stdout, re.MULTILINE)
                self.assertIsNotNone(unit_match, generated.stdout)
                runner = root / "runs" / f"{unit_match.group(1)}.sh"
                runner_text = runner.read_text(encoding="utf-8")
                runner.write_text(
                    runner_text.replace(
                        "PATH=/usr/sbin:/usr/bin:/sbin:/bin",
                        f"PATH={_posix_shell_path(fake_bin)}:/usr/sbin:/usr/bin:/sbin:/bin",
                        1,
                    ),
                    encoding="utf-8",
                )
                ...
                completed = subprocess.run(
                    [shell, _posix_shell_path(runner)],
                    env=os.environ.copy(),
                    text=True,
                    capture_output=True,
                    check=False,
                ...
                log_path = root / "runs" / f"{unit_match.group(1)}.log"
                log_lines = log_path.read_text(encoding="utf-8").splitlines()
                self.assertEqual([line for line in log_lines if line == "status=failed"], ["status=failed"])
                self.assertEqual([line for line in log_lines if line.startswith("error=")], [f"error={expected_error}"])
                self.assertNotIn("status=success", log_lines)
                self.assertFalse(
                    any(line in {"phase=staged", "phase=published", "phase=rollback", "phase=root", "phase=installed"} for line in log_lines),
                    log_lines,
                )
                if failure_case != "strict-stage-fetch":
                    self.assertNotIn("phase=verified", log_lines)
                git_calls = git_log.read_text(encoding="utf-8").splitlines()
                if failure_case == "remote-mismatch":
                    self.assertEqual(len(git_calls), 1)
                    self.assertIn("ls-remote", git_calls[0])
                elif failure_case == "verification-fetch":
                    self.assertEqual(sum(" fetch " in f" {call} " for call in git_calls), 1)
                else:
                    self.assertEqual(sum(" fetch " in f" {call} " for call in git_calls), 2)

    def test_generated_strict_runner_signal_contract_is_idempotent(self) -> None:
        helper = (Path(__file__).resolve().parents[1] / "scripts" / "gp-root-helper.sh").read_text(encoding="utf-8")
        runner_template = helper.split("exec > $(shell_quote \"$log_file\") 2>&1\n", 1)[1]

        self.assertIn("STRICT_TERMINAL_WRITTEN=0", runner_template)
        self.assertIn("STRICT_PUBLICATION_STATE=before", runner_template)
        self.assertIn("STRICT_SIGNAL_HANDLING=0", runner_template)
        self.assertIn("[ \"\\$STRICT_TERMINAL_WRITTEN\" = 0 ] || return 0", runner_template)
        self.assertIn("trap '' HUP INT TERM", runner_template)
        self.assertIn("trap 'strict_handle_signal HUP 129' HUP", runner_template)
        self.assertIn("trap 'strict_handle_signal INT 130' INT", runner_template)
        self.assertIn("trap 'strict_handle_signal TERM 143' TERM", runner_template)
        self.assertIn('STRICT_PUBLICATION_STATE=publishing\nrunuser -u "\\$STRICT_USER"', runner_template)
        self.assertIn("USER_PUBLISH\nSTRICT_PUBLICATION_STATE=published\necho 'phase=published'", runner_template)
        signal_handler = runner_template.split("strict_handle_signal() {", 1)[1].split("\n}\nstrict_install_signal_traps", 1)[0]
        prepublication_cleanup = runner_template.split("strict_clean_prepublication_transients() {", 1)[1].split("\n}\nstrict_git", 1)[0]
        self.assertIn("strict_clean_prepublication_transients", signal_handler)
        self.assertIn('received \\$strict_signal before publication', signal_handler)
        self.assertIn('[ -d "\\$install_dir/.git" ] && [ ! -L "\\$install_dir/.git" ]', prepublication_cleanup)
        self.assertIn('[ ! -e "\\$previous" ] && [ ! -L "\\$previous" ]', prepublication_cleanup)
        self.assertIn('[ "\\$publish_present:\\$next_present" != 1:1 ] || exit 126', prepublication_cleanup)
        self.assertIn('rm -rf -- "\\$next"', prepublication_cleanup)
        self.assertNotIn("restore_privileged_surface", prepublication_cleanup)

    def test_generated_strict_runner_signal_traps_roll_back_once_after_publication(self) -> None:
        if sys.platform != "linux" or getattr(os, "geteuid", lambda: -1)() != 0:
            self.skipTest("requires a Linux root runner; Windows does not model privileged strict update execution")
        shell = _posix_shell()
        if shell is None:
            self.skipTest("requires a POSIX sh interpreter")
        ...
        helper = Path(__file__).resolve().parents[1] / "scripts" / "gp-root-helper.sh"
        expected_sha = "a" * 40
        signal_cases = (("TERM", signal.SIGTERM, 143), ("HUP", signal.SIGHUP, 129), ("INT", signal.SIGINT, 130))
        cases = tuple(
            (phase_name, publication_state, observed_phase, rollback_expected, signal_name, signal_number, expected_returncode)
            for phase_name, publication_state, observed_phase, rollback_expected in (
                ("before-publication", "before", "requested", False),
                ("after-publication", "published", "published", True),
            )
            for signal_name, signal_number, expected_returncode in signal_cases
        )

        for phase_name, publication_state, observed_phase, rollback_expected, signal_name, signal_number, expected_returncode in cases:
            with self.subTest(phase=phase_name, signal=signal_name), tempfile.TemporaryDirectory(dir="/root") as raw:
                root = Path(raw)
                library = root / "root-helper-library.sh"
                library.write_text(_root_helper_test_source(helper).split("\nrequire_root\n", 1)[0] + "\n", encoding="utf-8")
                harness = '''\
...
'''
                generated = subprocess.run(
                    [shell, "-c", harness, "strict-runner-signal-generation", _posix_shell_path(library), _posix_shell_path(root), expected_sha],
                    env=os.environ.copy(),
                    text=True,
                    capture_output=True,
                    check=False,
                ...
                runner = root / "runs" / f"{unit}.sh"
                rollback_marker = root / "rollback-marker"
                injected_publication = (
                    "rollback_published_code() {\n"
                    "  echo 'phase=rollback'\n"
                    f"  printf '%s\\n' rollback > {shlex.quote(_posix_shell_path(rollback_marker))}\n"
                    "  return 0\n"
                    "}\n"
                    f"STRICT_PUBLICATION_STATE={publication_state}\n"
                    f"echo 'phase={observed_phase}'\n"
                    "while :; do sleep 1; done\n"
                )
                runner_text = runner.read_text(encoding="utf-8")
                self.assertIn('trap \'strict_handle_signal TERM 143\' TERM', runner_text)
                self.assertIn('echo "phase=requested"\n', runner_text)
                runner.write_text(runner_text.replace('echo "phase=requested"\n', injected_publication, 1), encoding="utf-8")

                running = subprocess.Popen([shell, _posix_shell_path(runner)], text=True)
                log_path = root / "runs" / f"{unit}.log"
                try:
                    deadline = time.monotonic() + 5
                    while time.monotonic() < deadline:
                        if f"phase={observed_phase}" in log_path.read_text(encoding="utf-8"):
                            break
                        time.sleep(0.02)
                    else:
                        self.fail(f"strict runner did not reach publication: {log_path.read_text(encoding='utf-8')}")
                    running.send_signal(signal_number)
                    self.assertEqual(running.wait(timeout=5), expected_returncode)
                finally:
                ...
                if rollback_expected:
                    self.assertEqual(rollback_marker.read_text(encoding="utf-8").splitlines(), ["rollback"])
                    self.assertEqual(
                        [line for line in log_lines if line.startswith("error=")],
                        [f"error=received {signal_name} after publication; exact legacy privileged surface was restored"],
                    )
                    self.assertLess(log_lines.index("phase=rollback"), log_lines.index("status=failed"))
                else:
                    self.assertFalse(rollback_marker.exists())
                    self.assertEqual(
                        [line for line in log_lines if line.startswith("error=")],
                        [f"error=received {signal_name} before publication"],
                    )
                    self.assertNotIn("phase=rollback", log_lines)

    def test_generated_strict_runner_signals_during_cleanup_and_after_irreversible_success(self) -> None:
        """A cleanup-time signal sees both rollback artifacts; a late one only exits."""
        if sys.platform != "linux" or getattr(os, "geteuid", lambda: -1)() != 0:
            self.skipTest("requires a Linux root runner; Windows does not model privileged strict update execution")
        shell = _posix_shell()
        if shell is None:
        ...
        for state, signal_name, signal_number, expected_returncode in (
            (state, signal_name, signal_number, expected_returncode)
            for state in ("cleanup", "after-success")
            for signal_name, signal_number, expected_returncode in signal_cases
        ):
            with self.subTest(state=state, signal=signal_name), tempfile.TemporaryDirectory(dir="/root") as raw:
                root = Path(raw)
                library = root / "root-helper-library.sh"
                library.write_text(_root_helper_test_source(helper).split("\nrequire_root\n", 1)[0] + "\n", encoding="utf-8")
                harness = '''\
...
'''
                generated = subprocess.run(
                    [shell, "-c", harness, "strict-runner-irreversible-signal-generation", _posix_shell_path(library), _posix_shell_path(root), expected_sha],
                    env=os.environ.copy(),
                    text=True,
                    capture_output=True,
                    check=False,
                ...
                snapshot = root / "legacy-privileged-surface"
                rollback_marker = root / "rollback-marker"

                if state == "cleanup":
                    injected = (
                        f"STRICT_PREVIOUS_DIR={shlex.quote(_posix_shell_path(previous))}\n"
                        f"STRICT_PRIVILEGED_SNAPSHOT={shlex.quote(_posix_shell_path(snapshot))}\n"
                        f"ROLLBACK_MARKER={shlex.quote(_posix_shell_path(rollback_marker))}\n"
                        'mkdir -p "$STRICT_PREVIOUS_DIR" "$STRICT_PRIVILEGED_SNAPSHOT"\n'
                        "STRICT_PUBLICATION_STATE=published\n"
                        "rollback_published_code() {\n"
                        "  [ -d \"$STRICT_PREVIOUS_DIR\" ] && [ -d \"$STRICT_PRIVILEGED_SNAPSHOT\" ] || return 1\n"
                    ...
                    injected = (
                        ...
                        "echo 'status=success'\n"
                        "echo 'phase=after-success'\n"
                        "while :; do sleep 1; done\n"
                    )
                    observed_phase = "after-success"
                ...
                try:
                    ...
                    while time.monotonic() < deadline:
                        ...
                        self.fail(f"strict runner did not reach {observed_phase}: {log_path.read_text(encoding='utf-8')}")
                    running.send_signal(signal_number)
                    self.assertEqual(running.wait(timeout=5), expected_returncode)
                finally:
                    if running.poll() is None:
                ...
                log_lines = log_path.read_text(encoding="utf-8").splitlines()
                if state == "cleanup":
                    self.assertEqual(rollback_marker.read_text(encoding="utf-8").splitlines(), ["previous+snapshot"])
                    self.assertEqual([line for line in log_lines if line == "status=failed"], ["status=failed"])
                    self.assertEqual(
                        [line for line in log_lines if line.startswith("error=")],
                        [f"error=received {signal_name} after publication; exact legacy privileged surface was restored"],
                    )
                    self.assertIn("phase=rollback", log_lines)
                    self.assertNotIn("status=success", log_lines)
                else:
                    self.assertFalse(rollback_marker.exists())
                    self.assertEqual([line for line in log_lines if line == "status=success"], ["status=success"])
                    self.assertEqual([line for line in log_lines if line == "status=failed"], [])
                    self.assertEqual([line for line in log_lines if line.startswith("error=")], [])
                    self.assertNotIn("phase=rollback", log_lines)

    def test_generated_strict_runner_ignores_late_signal_after_actual_rollback_cleanup(self) -> None:
        """Signals after USER_FINALIZE removed previous must complete the sole success."""
        if sys.platform != "linux" or getattr(os, "geteuid", lambda: -1)() != 0:
            self.skipTest("requires a Linux root runner; Windows does not model privileged strict update execution")
        shell = _posix_shell()
        if shell is None:
        ...
        helper = Path(__file__).resolve().parents[1] / "scripts" / "gp-root-helper.sh"
        expected_sha = "e" * 40
        signal_cases = (("TERM", signal.SIGTERM), ("HUP", signal.SIGHUP), ("INT", signal.SIGINT))

        for signal_name, signal_number in signal_cases:
            with self.subTest(signal=signal_name), tempfile.TemporaryDirectory(dir="/root") as raw:
                root = Path(raw)
                fake_bin = root / "fake-bin"
                fake_bin.mkdir()
                library = root / "root-helper-library.sh"
...
'''
                generated = subprocess.run(
                    [shell, "-c", harness, "strict-runner-post-cleanup-signal-generation", _posix_shell_path(library), _posix_shell_path(root), expected_sha],
                    env=os.environ.copy(),
                    text=True,
                    capture_output=True,
                    check=False,
                ...
                candidate.mkdir()
                previous.mkdir()
                (candidate / "candidate-marker").write_text("candidate\n", encoding="utf-8")
                barrier = root / "post-cleanup-barrier"
                go = root / "post-cleanup-go"
                ...
                (fake_bin / "runuser").chmod(0o700)
                (fake_bin / "systemctl").write_text("#!/bin/sh\nexit 0\n", encoding="utf-8")
                (fake_bin / "systemctl").chmod(0o700)

                runner_text = runner.read_text(encoding="utf-8").replace(
                    "PATH=/usr/sbin:/usr/bin:/sbin:/bin", f"PATH={_posix_shell_path(fake_bin)}:/usr/sbin:/usr/bin:/sbin:/bin", 1
                )
                preamble = runner_text.split('echo "phase=requested"\n', 1)[0]
                strict_functions = "rollback_published_code() {" + runner_text.split("rollback_published_code() {", 1)[1].split("STRICT_PUBLICATION_STATE=publishing\n", 1)[0]
                success_path = runner_text.split("echo 'phase=root'\n", 1)[1]
                self.assertIn("strict_begin_irreversible_cleanup\n", success_path)
                self.assertIn('rm -rf -- "$previous"', success_path)
                injected = (
                    f"STRICT_INSTALL_DIR={shlex.quote(_posix_shell_path(candidate))}\n"
                    f"STRICT_PREVIOUS_DIR={shlex.quote(_posix_shell_path(previous))}\n"
                    "STRICT_USER=gp\n"
                    "STRICT_CORE_SERVICE=gp-control-plane-core.service\n"
                    "STRICT_WEB_SERVICE=gp-control-plane-web.service\n"
                    ...
                    f"fetch_sha={expected_sha}\n"
                    f"stage_head={expected_sha}\n"
                    f"installed_checkout_sha={expected_sha}\n"
                    "STRICT_STATE_LAYOUT=external\n"
                    "run_staged_installer() { :; }\n"
                )
                runner_text = preamble + strict_functions + injected + success_path
                runner_text = runner_text.replace(
                    'rm -rf -- "$previous"',
                    'rm -rf -- "$previous"\nprintf \'removed\\n\' > "$GP_STRICT_TEST_POST_CLEANUP_BARRIER"\nwhile [ ! -e "$GP_STRICT_TEST_POST_CLEANUP_GO" ]; do sleep 1; done',
                    1,
                )
                ...
                running = subprocess.Popen(
                    ...
                    env={
                        **os.environ,
                        "GP_STRICT_TEST_POST_CLEANUP_BARRIER": _posix_shell_path(barrier),
                        "GP_STRICT_TEST_POST_CLEANUP_GO": _posix_shell_path(go),
                    },
                    text=True,
                )
                log_path = root / "runs" / f"{unit}.log"
                try:
                    ...
                    while time.monotonic() < deadline:
                        ...
                        self.fail(f"strict runner did not reach the post-cleanup barrier: {log_path.read_text(encoding='utf-8')}")
                    self.assertFalse(previous.exists(), "barrier must be immediately after the production rollback checkout removal")
                    self.assertTrue((candidate / "candidate-marker").is_file())
                    running.send_signal(signal_number)
                    go.touch()
                    self.assertEqual(running.wait(timeout=5), 0)
                finally:
                    if running.poll() is None:
                        running.kill()
                        running.wait(timeout=5)
                ...
                self.assertEqual([line for line in log_lines if line.startswith("error=")], [])
                self.assertNotIn("phase=rollback", log_lines)
                self.assertEqual((candidate / "candidate-marker").read_text(encoding="utf-8"), "candidate\n")

    def test_generated_strict_runner_signal_after_first_publication_move_cleans_only_transient_candidate(self) -> None:
        """After publish->next, signal cleanup must retain the untouched legacy checkout."""
        if sys.platform != "linux" or getattr(os, "geteuid", lambda: -1)() != 0:
            self.skipTest("requires a Linux root runner; Windows does not model privileged strict update execution")
        shell = _posix_shell()
        if shell is None or shutil.which("tar") is None:
            self.skipTest("requires POSIX sh and tar")
        ...
        for signal_name, signal_number, expected_returncode in signal_cases:
            with self.subTest(signal=signal_name), tempfile.TemporaryDirectory(dir="/root") as raw:
                ...
                fake_bin = root / "fake-bin"
                fake_bin.mkdir()
                install_dir = root / "legacy-install"
                install_dir.mkdir()
                (install_dir / ".git").mkdir()
                legacy_marker = install_dir / "legacy-marker"
                legacy_marker.write_text("legacy checkout\n", encoding="utf-8")
                legacy_marker.chmod(0o640)
                privileged_surface = root / "privileged-surface"
                privileged_surface.mkdir()
                privileged_file = privileged_surface / "root-helper"
                privileged_file.write_text("legacy privileged surface\n", encoding="utf-8")
                privileged_file.chmod(0o440)
                legacy_before = (legacy_marker.read_bytes(), legacy_marker.stat().st_mode & 0o777)
                privileged_before = (privileged_file.read_bytes(), privileged_file.stat().st_mode & 0o777)
                bundle_source = root / "bundle-source"
                (bundle_source / "scripts").mkdir(parents=True)
                (bundle_source / "scripts" / "install-linux.sh").write_text("#!/bin/sh\n", encoding="utf-8")
                bundle = root / "candidate.tar"
                subprocess.run(["tar", "-C", str(bundle_source), "-cf", str(bundle), "."], check=True, capture_output=True, text=True)
                barrier = root / "after-publish-to-next"

                def posix(path: Path) -> str:
                    return _posix_shell_path(path)
...
load_strict_install_profile() {
  strict_install_user=gp
  strict_install_dir="$3"
  strict_state_dir="$2/state"
  strict_state_layout=external
  strict_state_relative=
  strict_data_root=
  strict_core_service=gp-control-plane-core.service
...
chmod() { :; }
systemd-run() { :; }
queue_strict_update refs/heads/dev "$4"
'''
                generated = subprocess.run(
                    [shell, "-c", harness, "strict-runner-first-publication-move-generation", posix(library), posix(root), posix(install_dir), expected_sha],
                    env={**os.environ, "PATH": f"{posix(fake_bin)}:/usr/bin:/bin"},
                    text=True,
                    capture_output=True,
                    check=False,
                )
                ...
                runner = root / "runs" / f"{unit}.sh"
                runner_text = runner.read_text(encoding="utf-8").replace(
                    "PATH=/usr/sbin:/usr/bin:/sbin:/bin", f"PATH={posix(fake_bin)}:/usr/sbin:/usr/bin:/sbin:/bin", 1
                )
                preamble = runner_text.split('echo "phase=requested"\n', 1)[0]
                publication = "STRICT_PUBLICATION_STATE=publishing\n" + runner_text.split("STRICT_PUBLICATION_STATE=publishing\n", 1)[1].split("STRICT_PUBLICATION_STATE=published\n", 1)[0]
                self.assertIn('mv "$publish_dir" "$next"\nmv "$install_dir" "$previous" || exit 126', publication)
                injected = (
                    f"STRICT_USER=gp\nSTRICT_INSTALL_DIR={shlex.quote(posix(install_dir))}\n"
                    f"STRICT_PREVIOUS_DIR={shlex.quote(posix(root / '.legacy-install.strict-previous'))}\n"
                    f"STRICT_BUNDLE={shlex.quote(posix(bundle))}\n"
                    f"STRICT_PRIVILEGED_SNAPSHOT={shlex.quote(posix(privileged_surface))}\n"
                )
                runner_text = preamble + injected + publication
                runner_text = runner_text.replace(
                    'mv "$publish_dir" "$next"\nmv "$install_dir" "$previous" || exit 126',
                    'mv "$publish_dir" "$next"\nprintf \'after-first-move\\n\' > "$GP_STRICT_TEST_FIRST_PUBLICATION_BARRIER"\nwhile :; do sleep 1; done\nmv "$install_dir" "$previous" || exit 126',
                    1,
                )
                runner.write_text(runner_text, encoding="utf-8")
                ...
                running = subprocess.Popen(
                    [shell, posix(runner)],
                    env={**os.environ, "GP_STRICT_TEST_FIRST_PUBLICATION_BARRIER": posix(barrier)},
                    text=True,
                ...
                try:
                    ...
                    while time.monotonic() < deadline:
                        ...
                        self.fail("strict runner did not reach the first publication move boundary")
                    self.assertTrue(install_dir.is_dir())
                    self.assertTrue((root / ".legacy-install.strict-next").is_dir())
                    os.killpg(running.pid, signal_number)
                    self.assertEqual(running.wait(timeout=5), expected_returncode)
                finally:
                    if running.poll() is None:
                ...
                log_lines = (root / "runs" / f"{unit}.log").read_text(encoding="utf-8").splitlines()
                self.assertEqual([line for line in log_lines if line == "status=failed"], ["status=failed"])
                self.assertEqual([line for line in log_lines if line == "status=success"], [])
                self.assertEqual([line for line in log_lines if line.startswith("error=")], [f"error=received {signal_name} before publication"])
                self.assertNotIn("phase=rollback", log_lines)
                self.assertEqual((legacy_marker.read_bytes(), legacy_marker.stat().st_mode & 0o777), legacy_before)
                self.assertEqual((privileged_file.read_bytes(), privileged_file.stat().st_mode & 0o777), privileged_before)
                self.assertFalse((root / ".legacy-install.strict-publish").exists())
                self.assertFalse((root / ".legacy-install.strict-next").exists())
                self.assertFalse((root / ".legacy-install.strict-previous").exists())

    def test_generated_strict_runner_term_between_publication_moves_restores_checkout_and_surface(self) -> None:
        if sys.platform != "linux" or getattr(os, "geteuid", lambda: -1)() != 0:
            self.skipTest("requires a Linux root runner; Windows does not model privileged strict update execution")
        shell = _posix_shell()
        if shell is None:
        ...
        with tempfile.TemporaryDirectory(dir="/root") as raw:
            ...
            install_dir.mkdir()
            (install_dir / ".git").mkdir()
            (install_dir / "legacy-marker").write_text("legacy checkout\n", encoding="utf-8")
            barrier = root / "between-publication-moves"

            paths = {
                "root-helper": rootfs / "usr/local/libexec/gp-control-plane/gp-root-helper",
                "root-helper-config": rootfs / "etc/default/gp-control-plane-root-helper",
                "sudoers": rootfs / "etc/sudoers.d/gp-control-plane-root-helper",
                "install-profile": rootfs / "etc/default/gp-control-plane-install-profile",
                "core-env": rootfs / "etc/default/gp-control-plane-core",
                "web-env": rootfs / "etc/default/gp-control-plane-web",
                "core-unit": rootfs / "etc/systemd/system/gp-control-plane-core.service",
                "web-unit": rootfs / "etc/systemd/system/gp-control-plane-web.service",
            }
            for name, path in paths.items():
                path.parent.mkdir(parents=True, exist_ok=True)
                path.write_text(f"legacy-{name}\n", encoding="utf-8")
                path.chmod(0o440 if name == "sudoers" else 0o644)
            surface_before = {name: (path.read_bytes(), path.stat().st_mode & 0o777) for name, path in paths.items()}

            def posix(path: Path) -> str:
                return _posix_shell_path(path)

            candidate = root / "candidate-install-linux.sh"
            candidate.write_text(
                "#!/bin/sh\n"
                'case "${1:-}" in --strict-preflight) exit 0 ;; esac\n'
            ...
            (fake_bin / "git").write_text(
                ...
                "while [ \"$#\" -gt 0 ]; do\n"
                "  case \"$1\" in\n"
                "    -C|-c) [ \"$1\" = -C ] && repo=$2; shift 2 ;;\n"
                "    *) break ;;\n"
                "  esac\n"
                "done\n"
                "case \"$1\" in\n"
                "  check-ref-format) exit 0 ;;\n"
                f"  ls-remote) printf '%s\\t%s\\n' {expected_sha} refs/heads/dev ;;\n"
                "  init) /bin/mkdir -p \"$2/.git\" ;;\n"
                "  fetch)\n"
                "    case \"$repo\" in\n"
                f"      */repo) /bin/mkdir -p \"$repo/scripts\"; /bin/cp {shlex.quote(posix(candidate))} \"$repo/scripts/install-linux.sh\" ;;\n"
                "    esac\n"
                "    ;;\n"
                "  rev-parse)\n"
                "    case \"$repo\" in\n"
                f"      */legacy-install) printf '%s\\n' {previous_sha} ;;\n"
                f"      *) printf '%s\\n' {expected_sha} ;;\n"
                "    esac\n"
                "    ;;\n"
                "  checkout) exit 0 ;;\n"
            ...
            (fake_bin / "systemctl").write_text(
                "#!/bin/sh\n"
                "case \"$1\" in\n"
                "  daemon-reload|enable|disable|mask|unmask|restart|stop) exit 0 ;;\n"
                "  is-enabled) printf '%s\\n' disabled; exit 1 ;;\n"
                "  is-active) exit 3 ;;\n"
                "  *) exit 64 ;;\n"
                "esac\n",
...
STRICT_STAGE_DIR="$2/stages"
STRICT_BUNDLE_DIR="$2/bundles"
STRICT_INSTALL_PROFILE="$2/rootfs/etc/default/gp-control-plane-install-profile"
DISCOVERY_GATE_FILE="$2/discovery-update.lock"
load_strict_install_profile() {
  strict_install_user=gp
  strict_install_dir="$3"
  ...
  strict_core_service=gp-control-plane-core.service
  strict_web_service=gp-control-plane-web.service
  strict_install_web=on
  strict_core_env_file="$2/rootfs/etc/default/gp-control-plane-core"
  strict_web_env_file="$2/rootfs/etc/default/gp-control-plane-web"
}
prepare_strict_state_layout() { :; }
ensure_strict_root_dir() { /bin/mkdir -p "$1"; }
date() { printf '20260101T000000Z\\n'; }
...
'''
            generated = subprocess.run(
                [shell, "-c", harness, "strict-runner-middle-publication-generation", posix(library), posix(root), posix(install_dir), expected_sha],
                env={**os.environ, "PATH": f"{posix(fake_bin)}:/usr/bin:/bin"},
                text=True,
                capture_output=True,
                check=False,
            ...
            runner_text = runner.read_text(encoding="utf-8").replace(
                "PATH=/usr/sbin:/usr/bin:/sbin:/bin",
                f"PATH={posix(fake_bin)}:/usr/sbin:/usr/bin:/sbin:/bin",
                1,
            )
            for original, replacement in (
                ("/usr/local/libexec/gp-control-plane/gp-root-helper", posix(paths["root-helper"])),
                ("/etc/default/gp-control-plane-root-helper", posix(paths["root-helper-config"])),
                ("/etc/sudoers.d/gp-control-plane-root-helper", posix(paths["sudoers"])),
                ("/etc/default/gp-control-plane-install-profile", posix(paths["install-profile"])),
                ("/etc/default/gp-control-plane-core", posix(paths["core-env"])),
                ("/etc/default/gp-control-plane-web", posix(paths["web-env"])),
                ("/etc/systemd/system/gp-control-plane-core.service", posix(paths["core-unit"])),
                ("/etc/systemd/system/gp-control-plane-web.service", posix(paths["web-unit"])),
            ):
                runner_text = runner_text.replace(original, replacement)
            publication_move = 'mv "$install_dir" "$previous" || exit 126\nif ! mv "$next" "$install_dir"; then'
            self.assertIn(publication_move, runner_text)
            runner_text = runner_text.replace(
                publication_move,
                'mv "$install_dir" "$previous" || exit 126\nprintf \'between-moves\\n\' > "$GP_STRICT_TEST_PUBLICATION_BARRIER"\nwhile :; do sleep 1; done\nif ! mv "$next" "$install_dir"; then',
                1,
            )
            runner.write_text(runner_text, encoding="utf-8")
            ...
            running = subprocess.Popen(
                [shell, posix(runner)],
                env={**os.environ, "GP_STRICT_TEST_PUBLICATION_BARRIER": posix(barrier)},
                text=True,
                start_new_session=True,
            )
            try:
                ...
                while time.monotonic() < deadline:
                    ...
                    self.fail("strict runner did not reach the publication boundary")
                self.assertFalse(install_dir.exists(), "barrier must be exactly between the two publication moves")
                self.assertTrue((root / ".legacy-install.strict-previous").is_dir())
                self.assertTrue((root / ".legacy-install.strict-next").is_dir())
                os.killpg(running.pid, signal.SIGTERM)
                self.assertEqual(running.wait(timeout=5), 143)
            finally:
                if running.poll() is None:
                    os.killpg(running.pid, signal.SIGKILL)
                    running.wait(timeout=5)
            ...
            self.assertEqual(
                [line for line in log_lines if line.startswith("error=")],
                ["error=received TERM after publication; exact legacy privileged surface was restored"],
            )
            self.assertIn("phase=rollback", log_lines)
            self.assertIn("rollback_scope=legacy-privileged-surface", log_lines)
            self.assertEqual((install_dir / "legacy-marker").read_text(encoding="utf-8"), "legacy checkout\n")
            self.assertFalse((root / ".legacy-install.strict-previous").exists())
            self.assertFalse((root / ".legacy-install.strict-next").exists())
            self.assertTrue((root / ".legacy-install.strict-failed").is_dir())
            for name, path in paths.items():
                self.assertEqual((path.read_bytes(), path.stat().st_mode & 0o777), surface_before[name], name)

    def test_generated_strict_runner_restores_v034_web_only_privileged_surface_after_publication_failure(self) -> None:
        if os.name == "nt":
            self.skipTest("strict runner execution is verified on Linux boards; MSYS path translation is not a POSIX privilege model")
        shell = _posix_shell()
        if shell is None:
        ...
        if getattr(os, "geteuid", lambda: -1)() != 0:
            self.skipTest("requires real root-owned metadata; this is not a fake-root strict updater test")
        helper = Path(__file__).resolve().parents[1] / "scripts" / "gp-root-helper.sh"
        expected_sha = "a" * 40
        ...
        with tempfile.TemporaryDirectory(dir="/root") as raw:
            ...
            install_dir.mkdir()
            (install_dir / ".git").mkdir()
            (install_dir / "legacy-marker").write_text("v0.3.4 web-only\n", encoding="utf-8")
            ...
            legacy_files = {
                "root-helper": ("legacy-root-helper\n", 0o755),
                "root-helper-config": ("ZAPRET_DIR='/opt/zapret2'\n", 0o644),
                "sudoers": ("legacy ALL=(root) NOPASSWD: /legacy/root-helper *\n", 0o440),
                "install-profile": ("legacy-profile\n", 0o600),
                "web-env": ("GP_INSTALL_DIR='/legacy'\n", 0o640),
                "web-unit": ("[Service]\nExecStart=/legacy/web\n", 0o644),
            }
            for name, (content, mode) in legacy_files.items():
                path = paths[name]
                path.parent.mkdir(parents=True, exist_ok=True)
            ...
            for name in ("root-helper", "root-helper-config", "sudoers", "install-profile", "web-env", "web-unit"):
                candidate_lines.append(f"printf 'candidate-{name}\\n' > {shlex.quote(posix(paths[name]))}")
                candidate_lines.append(f"/bin/chmod 0666 {shlex.quote(posix(paths[name]))}")
            for name in ("core-env", "core-unit"):
                candidate_lines.append(f"printf 'candidate-{name}\\n' > {shlex.quote(posix(paths[name]))}")
                candidate_lines.append(f"/bin/chmod 0666 {shlex.quote(posix(paths[name]))}")
            candidate_lines.extend(
                [
                    "systemctl enable gp-control-plane-core.service",
                    "systemctl restart gp-control-plane-core.service",
                    "systemctl disable gp-control-plane-web.service",
                    "systemctl stop gp-control-plane-web.service",
                    "exit 1",
                ]
            )
            candidate_installer = root / "candidate-install-linux.sh"
            candidate_installer.write_text("\n".join(candidate_lines) + "\n", encoding="utf-8")
            candidate_installer.chmod(0o700)

            candidate_template = shlex.quote(posix(candidate_installer))
            (fake_bin / "git").write_text(
                "#!/bin/sh\n"
                "repo=\n"
                "while [ \"$#\" -gt 0 ]; do\n"
                "  case \"$1\" in -C) repo=$2; shift 2 ;; *) break ;; esac\n"
                "done\n"
                "case \"$1\" in\n"
                "  check-ref-format) exit 0 ;;\n"
                f"  ls-remote) printf '%s\\t%s\\n' {expected_sha} refs/heads/dev ;;\n"
                ...
                "  fetch)\n"
                "    case \"$repo\" in\n"
                f"      */repo) /bin/mkdir -p \"$repo/scripts\"; /bin/cp {candidate_template} \"$repo/scripts/install-linux.sh\" ;;\n"
                "    esac\n"
                "    ;;\n"
                f"  rev-parse) printf '%s\\n' {expected_sha} ;;\n"
                "  checkout) exit 0 ;;\n"
            ...
            (fake_bin / "runuser").chmod(0o700)
            systemctl_log_shell = shlex.quote(posix(systemctl_log))
            state_dir = root / "systemctl-state"
            state_dir.mkdir()
            (state_dir / "gp-control-plane-web.service").write_text("enabled-runtime:active\n", encoding="utf-8")
            state_dir_shell = shlex.quote(posix(state_dir))
            (fake_bin / "systemctl").write_text(
                "#!/bin/sh\n"
                f"printf '%s\\n' \"$*\" >> {systemctl_log_shell}\n"
                f"state_dir={state_dir_shell}\n"
                "unit_path() {\n"
                "  case \"$1\" in\n"
                ...
                "  esac\n"
                "}\n"
                "require_unit() { unit=$(unit_path \"$1\") || exit $?; [ -e \"$unit\" ] || [ -L \"$unit\" ] || exit 4; }\n"
                "read_state() { state_file=\"$state_dir/$1\"; [ -f \"$state_file\" ] || { enabled=disabled; active=inactive; return; }; IFS=: read -r enabled active < \"$state_file\"; }\n"
                "write_state() { printf '%s:%s\\n' \"$2\" \"$3\" > \"$state_dir/$1\"; }\n"
                "command=$1; shift\n"
                "case \"$command\" in\n"
                "  daemon-reload) exit 0 ;;\n"
                "  is-enabled)\n"
                "    require_unit \"$1\"; read_state \"$1\"; printf '%s\\n' \"$enabled\"\n"
                "    case \"$enabled\" in enabled|enabled-runtime) exit 0 ;; disabled|disabled-runtime|masked|masked-runtime) exit 1 ;; *) exit 65 ;; esac\n"
                "    ;;\n"
                "  is-active)\n"
                "    [ \"$1\" = --quiet ] && shift; require_unit \"$1\"; read_state \"$1\"; [ \"$active\" = active ] && exit 0; exit 3\n"
                "    ;;\n"
                "  enable|disable|mask|unmask|restart|stop)\n"
                "    runtime=; [ \"$1\" = --runtime ] && { runtime=runtime; shift; }; [ \"$1\" = --now ] && { now=1; shift; } || now=0\n"
                "    service=$1; require_unit \"$service\"; read_state \"$service\"\n"
                "    case \"$command\" in\n"
                "      enable) [ -n \"$runtime\" ] && enabled=enabled-runtime || enabled=enabled ;;\n"
                "      disable) [ -n \"$runtime\" ] && enabled=disabled-runtime || enabled=disabled; [ \"$now\" = 1 ] && active=inactive ;;\n"
                "      mask) [ -n \"$runtime\" ] && enabled=masked-runtime || enabled=masked ;;\n"
                "      unmask) [ -n \"$runtime\" ] && enabled=disabled-runtime || enabled=disabled ;;\n"
                "      restart) active=active ;;\n"
                "      stop) active=inactive ;;\n"
                "    esac\n"
                "    write_state \"$service\" \"$enabled\" \"$active\"\n"
                "    ;;\n"
                "  *) exit 64 ;;\n"
                "esac\n",
                encoding="utf-8",
...
'''
            generated = subprocess.run(
                [
                    ...
                    "-c",
                    harness,
                    "strict-runner-legacy-rollback-generation",
                    posix(library),
                    posix(root),
                    posix(install_dir),
                    expected_sha,
            ...
            completed = subprocess.run(
                ...
                env={
                    **os.environ,
                    "GP_FAKE_CORE_UNIT": posix(paths["core-unit"]),
                    "GP_FAKE_WEB_UNIT": posix(paths["web-unit"]),
                },
                text=True,
                capture_output=True,
                check=False,
            ...
            self.assertEqual(
                [line for line in log_lines if line.startswith("error=")],
                ["error=staged installer failed after publication; exact legacy privileged surface was restored"],
            )
            self.assertIn("phase=published", log_lines)
            self.assertIn("phase=root", log_lines)
            self.assertIn("phase=rollback", log_lines)
            self.assertIn("rollback_scope=legacy-privileged-surface", log_lines)
            self.assertNotIn("status=success", log_lines)

            for name, (content, mode) in legacy_files.items():
                self.assertEqual(paths[name].read_text(encoding="utf-8"), content, name)
                self.assertEqual(paths[name].stat().st_mode & 0o777, mode, name)
            self.assertFalse(paths["core-env"].exists())
            self.assertFalse(paths["core-unit"].exists())
            self.assertEqual((install_dir / "legacy-marker").read_text(encoding="utf-8"), "v0.3.4 web-only\n")
            self.assertEqual((state_dir / "gp-control-plane-web.service").read_text(encoding="utf-8"), "enabled-runtime:active\n")
            self.assertEqual((state_dir / "gp-control-plane-core.service").read_text(encoding="utf-8"), "disabled:inactive\n")

            systemctl_calls = systemctl_log.read_text(encoding="utf-8").splitlines()
            self.assertIn("disable --now gp-control-plane-core.service", systemctl_calls)
            self.assertIn("enable --runtime gp-control-plane-web.service", systemctl_calls)
            self.assertIn("restart gp-control-plane-web.service", systemctl_calls)
            rollback_calls = systemctl_calls[systemctl_calls.index("daemon-reload") + 1 :]
            self.assertFalse(any("gp-control-plane-core.service" in call for call in rollback_calls), rollback_calls)
            self.assertFalse(any(call == "stop gp-control-plane-web.service" for call in rollback_calls), rollback_calls)

    def test_generated_strict_runner_preserves_persistent_mask_and_reports_absent_core_quiesce_failure(self) -> None:
        if os.name == "nt":
            self.skipTest("strict runner execution is verified on Linux boards; MSYS path translation is not a POSIX privilege model")
        shell = _posix_shell()
        if shell is None:
        ...
        expected_sha = "b" * 40
        cases = (
            ("persistent-mask", True, "", "staged installer failed after publication; exact legacy privileged surface was restored"),
            ("absent-core-success", False, "", "staged installer failed after publication; exact legacy privileged surface was restored"),
            ("absent-core-disable-failure", False, "gp-control-plane-core.service", "staged installer failed after publication; legacy privileged surface rollback could not be completed"),
        )

        for case_name, persistent_mask, fail_disable_service, expected_error in cases:
            with self.subTest(case=case_name), tempfile.TemporaryDirectory(dir="/root") as raw:
                root = Path(raw)
                rootfs = root / "rootfs"
                fake_bin = root / "fake-bin"
                fake_bin.mkdir()
                ...
                if persistent_mask:
                    paths["core-unit"].parent.mkdir(parents=True, exist_ok=True)
                    paths["core-unit"].symlink_to("/dev/null")
                ...
                for name in ("root-helper", "root-helper-config", "sudoers", "install-profile", "web-env", "web-unit"):
                    candidate_lines.extend((
                        f"printf 'candidate-{name}\\n' > {shlex.quote(posix(paths[name]))}",
                        f"/bin/chmod 0666 {shlex.quote(posix(paths[name]))}",
                    ))
                for name in ("core-env", "core-unit"):
                    if name == "core-unit" and persistent_mask:
                        candidate_lines.append(f"/bin/rm -f {shlex.quote(posix(paths[name]))}")
                    candidate_lines.extend((
                        f"printf 'candidate-{name}\\n' > {shlex.quote(posix(paths[name]))}",
                        f"/bin/chmod 0666 {shlex.quote(posix(paths[name]))}",
                    ))
                ...
                (fake_bin / "git").write_text(
                    ...
                    "  fetch)\n"
                    "    case \"$repo\" in\n"
                    f"      */repo) /bin/mkdir -p \"$repo/scripts\"; /bin/cp {shlex.quote(posix(candidate_installer))} \"$repo/scripts/install-linux.sh\" ;;\n"
                    "    esac\n"
                    "    ;;\n"
                    f"  rev-parse) printf '%s\\n' {expected_sha} ;;\n"
                    "  checkout) exit 0 ;;\n"
                ...
                (state_dir / "gp-control-plane-web.service").write_text("enabled-runtime:active\n", encoding="utf-8")
                if persistent_mask:
                    (state_dir / "gp-control-plane-core.service").write_text("masked:inactive\n", encoding="utf-8")
                (fake_bin / "systemctl").write_text(
                    "#!/bin/sh\n"
                    f"printf '%s\\n' \"$*\" >> {shlex.quote(posix(systemctl_log))}\n"
                    f"state_dir={shlex.quote(posix(state_dir))}\n"
                    "unit_path() {\n"
                    "  case \"$1\" in\n"
                    "    gp-control-plane-core.service) printf '%s\\n' \"$GP_FAKE_CORE_UNIT\" ;;\n"
                    "    gp-control-plane-web.service) printf '%s\\n' \"$GP_FAKE_WEB_UNIT\" ;;\n"
                    ...
                    "    runtime=; [ \"$1\" = --runtime ] && { runtime=runtime; shift; }; [ \"$1\" = --now ] && { now=1; shift; } || now=0\n"
                    "    service=$1; require_unit \"$service\"; read_state \"$service\"\n"
                    "    [ \"$command\" = disable ] && [ \"$now\" = 1 ] && [ \"${GP_FAKE_FAIL_DISABLE_NOW_SERVICE:-}\" = \"$service\" ] && exit 73\n"
                    "    case \"$command\" in\n"
                    "      enable) [ -n \"$runtime\" ] && enabled=enabled-runtime || enabled=enabled ;;\n"
                    "      disable) [ -n \"$runtime\" ] && enabled=disabled-runtime || enabled=disabled; [ \"$now\" = 1 ] && active=inactive ;;\n"
                    "      mask)\n"
                    "        if [ -n \"$runtime\" ]; then enabled=masked-runtime; else enabled=masked; /bin/rm -f \"$unit\"; /bin/ln -s /dev/null \"$unit\"; fi\n"
                    "        ;;\n"
                    "      unmask)\n"
                    "        if [ -n \"$runtime\" ]; then enabled=disabled-runtime; else enabled=disabled; if [ -L \"$unit\" ] && [ \"$(readlink -- \"$unit\")\" = /dev/null ]; then /bin/rm -f \"$unit\"; fi; fi\n"
                    "        ;;\n"
                    "      restart) active=active ;;\n"
                    "      stop) active=inactive ;;\n"
                    "    esac\n"
...
'''
                generated = subprocess.run(
                    [shell, "-c", harness, "strict-runner-regression-generation", posix(library), posix(root), posix(install_dir), expected_sha],
                    env={**os.environ, "PATH": f"{posix(fake_bin)}:/usr/bin:/bin"},
                    text=True,
                    capture_output=True,
                    check=False,
                ...
                completed = subprocess.run(
                    ...
                    env={
                        **os.environ,
                        "GP_FAKE_CORE_UNIT": posix(paths["core-unit"]),
                        "GP_FAKE_WEB_UNIT": posix(paths["web-unit"]),
                        "GP_FAKE_FAIL_DISABLE_NOW_SERVICE": fail_disable_service,
                    },
                    text=True,
                    capture_output=True,
                    check=False,
                ...
                self.assertEqual(completed.returncode, 126, completed.stderr)
                log_lines = (root / "runs" / f"{unit_match.group(1)}.log").read_text(encoding="utf-8").splitlines()
                self.assertEqual([line for line in log_lines if line.startswith("error=")], [f"error={expected_error}"])
                self.assertIn("phase=rollback", log_lines)

                systemctl_calls = systemctl_log.read_text(encoding="utf-8").splitlines()
                if persistent_mask:
                    self.assertIn("stop gp-control-plane-core.service", systemctl_calls)
                    self.assertTrue(paths["core-unit"].is_symlink())
                    self.assertEqual(os.readlink(paths["core-unit"]), "/dev/null")
                    self.assertEqual((state_dir / "gp-control-plane-core.service").read_text(encoding="utf-8"), "masked:inactive\n")
                    rollback_calls = systemctl_calls[systemctl_calls.index("daemon-reload") + 1 :]
                    self.assertNotIn("unmask gp-control-plane-core.service", rollback_calls)
                    self.assertNotIn("stop gp-control-plane-core.service", rollback_calls)
                    self.assertIn("rollback_scope=legacy-privileged-surface", log_lines)
                else:
                    self.assertIn("disable --now gp-control-plane-core.service", systemctl_calls)
                    if fail_disable_service:
                        self.assertNotIn("rollback_scope=legacy-privileged-surface", log_lines)
                        self.assertTrue(paths["core-unit"].is_file())
                        self.assertIn("candidate-core-unit", paths["core-unit"].read_text(encoding="utf-8"))
                    else:
                        self.assertIn("rollback_scope=legacy-privileged-surface", log_lines)
                        self.assertFalse(paths["core-unit"].exists())
                        self.assertEqual((state_dir / "gp-control-plane-core.service").read_text(encoding="utf-8"), "disabled:inactive\n")

    def test_strict_runner_limits_unit_links_and_restores_runtime_systemd_states(self) -> None:
        helper = (Path(__file__).resolve().parents[1] / "scripts" / "gp-root-helper.sh").read_text(encoding="utf-8")

        unit_link_guard = helper.split("strict_unit_is_allowed_mask_link() {", 1)[1].split("\n}\n", 1)[0]
        unit_snapshot = helper.split("strict_snapshot_unit() {", 1)[1].split("\n}\n", 1)[0]
        service_snapshot = helper.split("strict_systemctl_enabled_state() {", 1)[1].split("\n}\n", 1)[0]
        service_restore = helper.split("strict_restore_service_state() {", 1)[1].split("\n}\n", 1)[0]

        self.assertIn('"\\$STRICT_SNAPSHOT_CORE_UNIT"|"\\$STRICT_SNAPSHOT_WEB_UNIT"', unit_link_guard)
        self.assertIn('= /dev/null', unit_link_guard)
        self.assertIn('readlink -f -- "\\$strict_unit_target"', unit_link_guard)
        self.assertNotIn("ln -s -- \\$(", unit_link_guard)
        self.assertIn("mask-link", unit_snapshot)
        self.assertIn("unit=absent\\nenabled=masked-runtime\\nactive=inactive", helper)
        for state in ("enabled-runtime", "disabled-runtime", "masked-runtime", "masked"):
            self.assertIn(state, service_snapshot)
            self.assertIn(state, service_restore)
        self.assertIn('systemctl enable --runtime "\\$strict_service_name"', service_restore)
        self.assertIn('systemctl disable --runtime "\\$strict_service_name"', service_restore)
        self.assertIn('systemctl mask --runtime "\\$strict_service_name"', service_restore)
        self.assertIn('strict_unit_is_allowed_mask_link "\\$strict_service_unit"', service_restore)
        self.assertIn('[ "\\$(strict_systemctl_enabled_state "\\$strict_service_name")" = "\\$strict_service_enabled" ]', service_restore)

...
class InstallerTests(unittest.TestCase):
    def test_installer_configures_root_helper(self) -> None:
        ...
        self.assertIn("GP_INSTALL_USER", helper)
        self.assertIn("safe.directory", helper)
        self.assertIn("repo_git", installer)
        self.assertIn('installed_ref="\\$(git', helper)
        self.assertIn('echo "installed_ref=\\$installed_ref"', helper)
        self.assertIn("awk '{print \\$NF}'", helper)
